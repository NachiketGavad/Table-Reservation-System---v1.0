<?php
	...
	if(ISSET($_POST['add_guest'])){
		...
		$query2 = $conn->query("SELECT * FROM `transaction` WHERE `date` = '$checkin'  && `status` = 'Pending'") or die(mysqli_error());
		$row = $query2->num_rows;
				...
				if($row > 0){
					echo "<div class = 'col-md-4'>
						...
						if($guest_id = $fetch['guest_id']){
							...
							$conn->query("INSERT INTO `transaction`(guest_id, status, date) VALUES('$guest_id', 'Pending', '$checkin')") or die(mysqli_error());
