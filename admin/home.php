...
<body>
	...
	<div style = "margin-left:10px;" class = "container">
		<strong><h3>Table Allotment</h3></strong>
	</div>
	<br />
	<div class = "container-fluid">	
		<div class = "panel panel-default">
			...
			<div class = "panel-body">
				...
				<table id = "table" class = "table table-bordered">
					...
					<tbody>
						<?php
							$query = $conn->query("SELECT * FROM `transaction` NATURAL JOIN `guest` WHERE `status` = 'Pending'") or die(mysqli_error());
							while($fetch = $query->fetch_array()){
						?>
						<tr>
							<td><?php echo $fetch['firstname']." ".$fetch['lastname']?></td>
							<td><?php echo $fetch['contactno']?></td>
							<td><strong><?php if($fetch['date'] <= date("Y-m-d", strtotime("+8 HOURS"))){echo "<label style = 'color:#ff0000;'>".date("M d, Y", strtotime($fetch['date']))."</label>";}else{echo "<label style = 'color:#00ff00;'>".date("M d, Y", strtotime($fetch['date']))."</label>";}?></strong></td>
